...
public class SeasonService : ISeasonService
{
	...
	{
		return await seasons
			.Find(tenant.Filter<Season>(season => season.Id == id))
	...
	{
		return await seasons
			.Find(tenant.Filter<Season>(season => season.IsActive))
			.FirstOrDefaultAsync(cancellationToken);
	}
	...
	)
	{
		...
		await seasons.InsertOneAsync(season, cancellationToken: cancellationToken);
	...
	)
	{
		...
		var result = await seasons.ReplaceOneAsync(
			tenant.Filter<Season>(existingSeason => existingSeason.Id == season.Id),
			season,
	...
	)
	{
		...
		return await seasons.FindOneAndUpdateAsync(
			tenant.Filter<Season>(currentSeason => currentSeason.Id == id),
			update,
	...
	private async Task DeactivateAllAsync(CancellationToken cancellationToken)
	{
		...
		await seasons.UpdateManyAsync(
			tenant.Filter<Season>(),
			update,
			cancellationToken: cancellationToken
		);
	}
	...
	)
	{
		...
		await seasons.UpdateManyAsync(
			tenant.Filter<Season>(season => season.Id != activeSeasonId),
			update,
			cancellationToken: cancellationToken
		);
	}
