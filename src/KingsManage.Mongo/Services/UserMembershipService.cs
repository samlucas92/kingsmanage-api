...
public sealed class UserMembershipService : IUserMembershipService
{
	...
	{
		var clubs = await this.clubs.Find(club => club.OrganizationId == tenant.OrganizationId && club.IsActive)
			.SortBy(club => club.Name).ToListAsync(cancellationToken);
		var clubIds = clubs.Select(club => club.Id).ToList();
		var teams = await this.teams.Find(team => team.OrganizationId == tenant.OrganizationId && clubIds.Contains(team.ClubId) && team.IsActive)
	...
	{
		var user = await users.Find(existing => existing.Id == userId && existing.Memberships.Any(membership => membership.OrganizationId == tenant.OrganizationId))
		...
		var wasOrganizationAdmin = user.Memberships.Any(membership =>
			membership.OrganizationId == tenant.OrganizationId && membership.Role == TenantRole.OrganizationAdmin);
		...
		{
			var adminCount = await users.CountDocumentsAsync(existing => existing.IsActive && existing.Memberships.Any(membership =>
				membership.OrganizationId == tenant.OrganizationId && membership.Role == TenantRole.OrganizationAdmin), cancellationToken: cancellationToken);
			if (adminCount <= 1) throw new InvalidOperationException("The final Organization Admin cannot be removed.");
		}

		var otherOrganizations = user.Memberships.Where(membership => membership.OrganizationId != tenant.OrganizationId);
		user.Memberships = otherOrganizations.Concat(normalised).ToList();
		user.DefaultOrganizationId = tenant.OrganizationId;
		user.DefaultClubId = defaultClubId ?? normalised.Select(membership => membership.ClubId).FirstOrDefault(id => id.HasValue) ?? tenant.ClubId;
	...
	private async Task ValidateAsync(List<UserMembership> memberships, Guid? defaultClubId, CancellationToken cancellationToken)
	{
		...
		var clubs = await this.clubs.Find(club => club.OrganizationId == tenant.OrganizationId && club.IsActive).ToListAsync(cancellationToken);
		var clubIds = clubs.Select(club => club.Id).ToHashSet();
		var teams = await this.teams.Find(team => team.OrganizationId == tenant.OrganizationId && team.IsActive).ToListAsync(cancellationToken);
